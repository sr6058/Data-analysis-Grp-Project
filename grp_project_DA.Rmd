...
#Smriti: Added logit model analysis for the main regression


Logit model analysis:
```{r}
logit.fit.1 = glm(educated_spef1 ~ health + labor + yearbirth + isfemale+ marriage,
                  data=merged_data_drop,
                  family=binomial(link=logit) )

logit.fit.clog = glm(educated_spef1 ~ health + labor + yearbirth + isfemale+ marriage,
                  data=merged_data_drop,
                  family=binomial(link="cloglog") )

stargazer(logit.fit.1, logit.fit.clog, type = "text", 
          title = "Regression Results",
          header = FALSE, digits = 2,
          dep.var.caption = "Dependent Variable: education",
          dep.var.labels.include = FALSE, single.row=TRUE, font.size = "small")

```

# Maitha's Code

setwd("~/Documents/Spring'23/Data Anlaysis/Project")
library(tidyverse)
library(stargazer)
library(lmtest)
library(gt)
...
nlabour <- subset(labour, select=c("hhid", "zone","lga", "state", "sector", "indiv" ,"ea", "s04aq05", "s04aq43"))

nroster <- subset(roster, select=c("hhid", "zone","lga", "state", "sector", "indiv" ,"ea", "s01q02", "s01q07"))
...
merged_data <- inner_join(neduc, nhealth,by=c('hhid', 'zone',
  ...
  # Rename
  merged_data <- merged_data %>% 
  rename(education0= s02q08, educ.s02q13= s02q13, health=s03q24, gender0=s01q02, marriage=s01q07, labour.hrs=s04aq05, labour.wage=s04aq43) 
  
  # Filtering
  #Creating a binary variable for gender, instead of being levels
merged_data$gender <- ifelse(merged_data$gender0 == 2, 1, 0)

#Creating the first binary education variable
...
#Dropping those who are done with the school system (For education) + who are too young to walk(For health) + who have informal relationships (For marraige)

merged_data1 <- subset(merged_data, educ.s02q13 != 1 & educ.s02q13 != 15, health !=-1)
merged_data1 <- subset(merged_data, labour.wage != 0)
merged_data1 <- subset(merged_data, marriage != 3)

#Dropping NA observations
merged_data1 <- merged_data1 %>% drop_na(education0, health, labour.hrs, gender0, marriage,labour.wage) 
...
merged_data1$marriage.fct <- ifelse(merged_data1$marriage %in% current_levels[1:2], new_levels[2], ifelse(merged_data1$marriage %in% current_levels[4:6], new_levels[3],0))
...
# Outliers
#Box plot of labour.hrs and labour.wage
...
#Removing the outliers of labour.wage that have a number > 100000

length(which(merged_data1$labour.wage > 1000000)) 
merged_data1 <- subset(merged_data1, labour.wage < 1000000)
...
regression0 <- lm(education ~ health + labour.hrs + labour.wage + marriage.fct + gender, data=merged_data1)
summary(regression0)

#Running the first specification, with an interaction term

regression1 <- lm(education ~ health + labour.hrs + labour.wage + marriage.fct + gender + marriage.fct*gender, data=merged_data1)
summary(regression1)

#Running the alternative specification, without an interaction term

regression2 <- lm(education.alt ~ health + labour.hrs + labour.wage + marriage1 + gender, data=merged_data1)
summary(regression2)

#Running the alternative specification, without an interaction term

regression3 <- lm(education.alt ~ health + labour.hrs + labour.wage + marriage1 + gender + marriage1*gender, data=merged_data1)
summary(regression3)
...
linearHypothesis(regression1, c("marriage1=0", "health=0", "gender=0", "labour.wage=0", "labour.hrs=0"), white.adjust = "hc1")
summary(regression1)$fstatistic 


# LPM/ LOGIT

lpm <- lm(education ~ health + labour.hrs + labour.wage + marriage1 + gender + marriage1*gender, data=merged_data1)
summary(lpm)

logit <- glm(education ~ health + labour.hrs + labour.wage + marriage1 + gender + marriage1*gender, data=merged_data1, family=binomial(link='logit'))
summary(logit) 

logitmfx(education ~ health + labour.hrs + labour.wage + marriage1 + gender + marriage1*gender, data=merged_data1, atmean=TRUE) 

# sexy plot
#Creating a visualization for regression
# DROP NAS
merged_data_new <- merged_data_new %>% drop_na(education, health, labor.hours, gender1, marriage.fct,labor.wages) 
# regression
softlaunch<-lm(maineducation~gender1+labor.hours+labor.wages+marriage.fct+ health + marriage.fct*gender1, data=merged_data_new)
...
Smriti : Changes with log variable:
...
#Dropping those who are done with the school system (For education) + who are too young to walk(For health) + who have informal relationships (For marriage)

merged_data1 <- merged_data[!(merged_data$educ.s02q13==1 | merged_data$educ.s02q13==15 | merged_data$health==-1| merged_data$labour.wage==0| merged_data$marriage==3),]
...
merged_data1$health <- ifelse(merged_data1$health %in% current_levelshealth[2], new_levelshealth[1], ifelse(merged_data1$health %in% current_levelshealth[3],new_levelshealth[2], ifelse(merged_data1$health %in% current_levelshealth[4],new_levelshealth[3],0)))
